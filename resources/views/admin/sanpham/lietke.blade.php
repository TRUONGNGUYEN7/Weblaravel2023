                    ...
                    <div class="table-responsive">
                         <table class="table table-striped b-t b-light">
                              ...
                              <tbody>
                                   ...
                                   <tr>
                                        <td class="table-plus">{{$item -> idsp}}</td>
                                        <td>{{$item -> tensp}}</td>
                                        <td>
                                             <img style="width:80px" src="/hinhanh/{{$item -> hinhanh}}" alt="">
                                         </td>
                                        <td class="table-plus">{{$item -> motasp}}</td>
                                        <td class="table-plus">{{$item -> tendanhmuc}}</td>
                                        <td class="table-plus">{{$item -> tenth}}</td>
                                        <td>
                                             <div style="width: 30px;">
                                                  @if ($item -> status == 1)
                                                  <a href="{{URL::to('/admin/sanpham/hiden/'.$item -> idsp)}}">
                                                       <input type="checkbox" checked name="hienthi" class="switch-btn" data-size="small" data-color="#0099ff">
                                                  </a>
                                                  @else
                                                  <a href="{{URL::to('/admin/sanpham/show/'.$item -> idsp)}}">
                                        ...
                                        <td>
                                             <table>
                                                  <tr>
                                                       <td>
                                                            <a href="{{URL::to('admin/sanpham/sua/'.$item -> idsp)}}"><i class="dw dw-edit"></i> Edit</a>

                                                       </td>
                                                       <td>
                                                            <form action="{{URL::to('/admin/sanpham/xoa/'.$item -> idsp)}}" method="POST">
                                                                 @csrf
